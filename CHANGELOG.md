...
- Tapping a sound on Settings → Alarm sounds now plays it, so you can hear what you're checking: the sound at its natural speed and pitch, on the alarm stream at your alarm volume, looped for up to 10 seconds. Tapping it again stops it, tapping another sound switches to that one, and leaving the page (or the app) stops it; each tap of "Siren sweep" plays a freshly drawn siren, as each alarm does. The play/stop icon at the start of each row shows which sound is playing, and checking or unchecking a sound is now its checkbox alone. `SoundPreviewer` (`DeviceSoundPreviewer`, over the ringing player's own sources, which moved into the shared `openSoundOutput`) does the playing; `AlarmSoundsViewModel` runs one preview at a time.
- Settings → Alarms' "Alarm sounds" is now a page of its own with a checkbox per sound, replacing the "All / Bundled only / System only" chips. The sounds come in three groups — **Device alarm sounds** (every alarm ringtone the phone offers), **Bundled sounds** (the eight OGGs shipped in the app) and **Siren** (the sweep the app generates itself) — and each group's header carries a checkbox that checks or unchecks the whole group at once (partly checked while the group is mixed; tapping it then checks the rest). An alarm draws at random only from the checked sounds, and a source with nothing left checked drops out of the draw with its share going to the others; with nothing checked anywhere the siren still rings, so an alarm is never silent. The Settings row reads "All sounds on" or "3 sounds off". The choice is stored as the sounds turned *off* (`Settings.disabledAlarmSounds`, under `disabled_alarm_sounds`), so a ringtone a system update adds, or an OGG a later app update adds, rings until you uncheck it. The old `sound_pool` choice is no longer read: anyone who had "Bundled only" or "System only" set starts from every sound on and unchecks a group instead. The ringtone list moved out of the ringing player into `SoundCatalogSource`, which the player and the page share, so the page lists exactly what an alarm can play. Review follow-ups: opening the page drops any unchecked id the device no longer has (a ringtone a system update removed), so the Settings row's count can't be stuck on a sound no checkbox can reach; a device whose ringtone list can't be read shows the siren alone rather than crashing (and prunes nothing); the next edit of the off set removes the old `sound_pool` key; and the group header no longer claims a heading TalkBack would never read.
...
- Alarms no longer keep ringing for a meeting's **old** time after its schedule changed out from under you. Two gaps closed. First, a meeting **moved** while the app was away (the background check found it and notified you) kept its alarm at the old time until the app was next opened: the automatic re-time (`MaintainAlarms`) only ran on the app's own foreground reload and after boot. Every background change check (`CalendarChangeWorker`) now runs it first, so the same check that reports the move re-times the alarm. Second, a meeting whose old identity is **gone** from the calendar — the organizer moved the whole series to new times (every occurrence gets a new key), deleted and re-sent the invite, or cancelled it — kept its alarm for good: the automatic re-time deliberately leaves a vanished event's alarm alone (a hedge against a sync hiccup), and "Set alarms" fell back to the stored times for a selection the provider no longer had, so even after you tapped the replacement and set alarms again, the old alarm stayed armed and rang. Now, when the "Set alarms" tap could read the calendar, a selection whose meeting isn't there any more has its alarm cancelled, is counted with the "declined or cancelled" skips in the snackbar, and is **dropped from the day's picks** (`reconcileAlarms`' `providerRead`, `AlarmReconciliation.vanished`): nothing on screen could deselect it, and left in place its stored time would still have gone out in the share text and the busy-calendar sync, so the "Your picks changed since you shared" banner now prompts the re-share that takes the old time out. A day whose only pick vanished goes back to "nothing picked" rather than sitting in "alarms set". When the calendar can't be read, the stored times still stand, as before. One consequence of the background re-time to be aware of: a meeting pulled forward inside the lead time, or dragged so that it is already in progress, has always been re-timed to an alarm that rings at once (a late nudge beats silence); that can now happen from a background check too, not only when the app is opened. `AlarmReconcilerTest`, `ScheduleAlarmsSideEffectsTest` and `CalendarChangeWorkerTest` gained cases for each.
...
- The loud full-screen schedule-change alert is now **opt-in**: Settings gains a "Schedule changes" section with "Loud schedule-change alerts", off by default. Off, a change to today found after you shared it only raises the ordinary "Your schedule changed since you shared it" notification (which makes its own sound, as it did before the alert existed); on, it rings full-screen exactly as in v1.0.20, under the same rules (today only, background checks only, never while the app is on screen). Anyone updating from v1.0.20 has to turn it on to keep the ringing. Meeting alarms are unaffected — they always ring. `Settings.loudChangeAlerts` (stored under `loud_change_alerts`) gates `ChangeMonitor`'s `loud`; turning it off doesn't stop an alert that is already ringing. Review follow-ups: the toggle's description reads "When off, you only get the notification", and `SettingsChipsSemanticsTest` pins the row as a switch that is off by default and reports its tap.
