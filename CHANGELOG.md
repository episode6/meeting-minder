...
- Compact (one-line) event chips give the whole line to the title when title and time range can't share it: the time range gives way to the start time alone, and then is dropped, instead of the title being ellipsized, since the chip's place on the hour grid already shows when the event is. An armed chip still shows its alarm time.
- Chip times that fall on the hour read "9a" / "12p" (12-hour devices) instead of "9:00" / "12:00", so a time range takes less of the chip.
