...
- Compact (one-line) event chips give the whole line to the title when title and time range can't share it: the time range is dropped instead of the title being ellipsized, since the chip's place on the hour grid already shows when the event is. An armed chip still shows its alarm time.
