...
- Stack review fixes (the outstanding `@claude review` findings on PR-4 through PR-14, in one follow-up PR):
  ...
  - **Sharing:** a fast double tap on "Share schedule" opens one chooser, not two (a share already pending is never replaced). The app-bar subtitle names the day when a schedule was shared on a different day ("shared Sep 13, 9:00 PM" on tomorrow's page), so a bare time can't read as that day's evening.
