...
internal class FakeSettingsRepository(initial: Settings = Settings()) : SettingsRepository {
    ...
    override suspend fun setBusySyncEnabled(enabled: Boolean) {
        settings.value = settings.value.copy(busySync = settings.value.busySync.copy(enabled = enabled))
    }
    ...
    override suspend fun setBusySyncFirstName(firstName: String) {
        settings.value = settings.value.copy(busySync = settings.value.busySync.copy(firstName = firstName.trim()))
    }
