 ...
 * so it takes as little of the chip as possible (unless the locale's AM and PM markers
 * start with the same letter, as in Japanese or Korean, where the short form would say
 * nothing). The 24-hour forms are zero-padded in both places so the gutter and the chips
 * read alike.
 */
@Immutable
class TimelineTimeFormat(is24Hour: Boolean, private val locale: Locale) {
    ...
    private val onTheHourFormatter = when {
        is24Hour || periodLetter(LocalTime.MIDNIGHT) == periodLetter(LocalTime.NOON) -> null
        else -> DateTimeFormatter.ofPattern("h", locale)
    }
    ...
    fun time(time: LocalTime): String = when {
        ...
        onTheHourFormatter != null && time.minute == 0 -> time.format(onTheHourFormatter) + periodLetter(time)
        else -> time.format(timeFormatter)
    }

    /** Like [time] but never shortened, "9:00" / "09:00" — for the ringing screen's clock, a standalone time with no gutter. */
    fun clockTime(time: LocalTime): String = time.format(timeFormatter)

    private fun periodLetter(time: LocalTime): String = time.format(periodFormatter).lowercase(locale).take(1)
