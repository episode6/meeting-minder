 ...
 * so in a snackbar. The write changes `SELF_ATTENDEE_STATUS` at once, so the day is
 * reloaded straight away ([CalendarContentChanged]) rather than waiting on the debounced
 * `ContentObserver`, and a "Yes" is also reported as [RsvpAccepted] so an armed selection
 * gets its "sent" tick. Failures are one snackbar, never retried.
 */
@ContributesTo(AppScope::class)
interface RespondToEventSideEffects {
    @OptIn(ExperimentalCoroutinesApi::class)
    @Provides @IntoSet
    fun respondToEvent(repository: CalendarRepository): SideEffect<AppState> = sideEffect {
