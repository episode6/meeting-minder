 ...
 * `selected_event` + the `SCHEDULED` rows of `scheduled_alarm` into [AppState.dayPlans] on
 * every change, forever (there is no `LoadDay`-style gate — the tables are tiny and every
 * screen may need any date). This is
...
interface ObserveDayPlansSideEffects {
    @Provides @IntoSet
    fun observeDayPlans(dao: DayPlanDao, alarmDao: ScheduledAlarmDao): SideEffect<AppState> = sideEffect {
        merge(
            actions.filter { false },
            combine(dao.observeDayPlans(), dao.observeSelectedEvents(), alarmDao.observeScheduled()) { plans, selections, scheduled ->
                SetDayPlans(buildDayPlans(plans, selections, scheduled))
            },
        )
    }
}
