 ...
 * | Ignored | title/colour/attendee edits, rewrites with identical times, events already over, all-day events |
 *
...
object ChangeDetector {
    ...
    fun detect(date: LocalDate, baseline: List<SnapshotEvent>, fresh: List<CalendarEvent>, now: Instant): List<ScheduleChange> {
        ...
        for (event in fresh) {
            if (event.key !in before && event.isMeeting && event.begin >= now) {
