...
- Reads **every calendar on every account** through the Android Calendar Provider. The only thing it ever writes back is your **RSVP**: setting alarms marks each chosen meeting "Yes, going", and a chip's long-press menu writes the Yes / No / Maybe you pick for that one occurrence.
...
| `data/calendar/` | `CalendarRepository` interface + `ContentResolverCalendarRepository`; `CalendarSyncRequester` (the Refresh button's `requestSync`) |
