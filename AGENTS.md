...
| Text width in a Robolectric compose test | Legacy Robolectric graphics measure every glyph at about a pixel, so a test that depends on text *not* fitting (ellipsis, an overflow branch such as the compact chip dropping its time range) passes vacuously. Annotate the test class `@GraphicsMode(GraphicsMode.Mode.NATIVE)` for real font metrics (`EventChipCompactTest`). |
