...
The only thing the app ever writes to the calendar is "Yes, going" for an event it just armed (TODO.md §4.6): `CalendarRepository.acceptInstance(event)`, called by `RsvpAcceptSideEffects` for each `RsvpAccept(date, key)` the alarm reconcile fans out. The pure `rsvpDecision(event)` (`model/Rsvp.kt`) is the §4.6 skip table and returns the `RsvpState` a newly armed selection starts in; only `PENDING` gets written. Rules that are easy to break: the decision runs only on the explicit "Set alarms" tap, never from a background reconcile; the write is addressed by `CalendarEvent.eventId`, never `key.eventId`; `Events.SELF_ATTENDEE_STATUS` is only writable through the `exception/{eventId}` insert (a recurring occurrence) — a plain event goes through our own `attendees/{selfAttendeeId}` row; and nothing is ever reversed (deselecting cancels the alarm and leaves the RSVP). `selected_event.rsvp_state` tracks the outcome and `rsvp_event_id` the row it went to (the new exception's id), which is what the `DIRTY == 0` promotion to `SYNCED` checks on the next reload of the day. The decision is written through `DayPlanDao.recordRsvpDecision`, which leaves an `ACCEPTED_LOCALLY`/`SYNCED` row alone so a re-armed selection keeps its "sent" tick; a selection toggled off loses its row (and so its tick) by design.
...
| RSVP without the skip table | Never call `acceptInstance` for an event whose `rsvpDecision` isn't `PENDING`: the exception insert crashes the provider ("Status update WTF") when the event has no self-attendee row, and a read-only calendar takes the local write only for the server to reject it on sync. |
