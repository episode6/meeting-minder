...
| `data/calendar/` | `CalendarRepository` interface + `ContentResolverCalendarRepository`; `CalendarSyncRequester` (the Refresh button's `requestSync`); `BusyCalendars.kt` (`writable`/`defaultBusyCalendar`/`effectiveBusyCalendar`, TODO.md §4.7) |
