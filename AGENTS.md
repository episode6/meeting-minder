...
| `store/sideeffects/` | One `SideEffect<AppState>` per concern (loading, toggling, alarms, RSVP — the automatic `RsvpAccept` and the menu's `RespondToEvent` — sharing, change detection, the Refresh button's `RefreshCalendars`) |
